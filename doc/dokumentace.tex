...
\usepackage{geometry}
\geometry{outer=1.1cm,top=1.5cm,bottom=2cm}
\usepackage{tikz}
\usepackage{enumitem}
\graphicspath{ {img/} }
...
\newenvironment{warningBox}
  ...
  \newcommand*\lawHead{\fontsize{7pt}{8.4pt}}
...
\begin{document}
    ...
    \section{Instalace}
    \begin{enumerate}
        \item \begin{tabularx}{\textwidth}{l X}
            \includegraphics[scale=1.5]{installer_button} & Poklepejte na soubor \emph{\appname{}\_setup.exe}.
            \end{tabularx}
        \item Budete požádání o~povolení aplikace k~provádení změn; stiskněte ano.
        \begin{center}
            \begin{annotatedFigure}
                {\includegraphics[width=0.75\linewidth]{installer_UAC}}
                \draw[red,thick,rounded corners] (0.04,0.06) rectangle (0.51,0.18);
            \end{annotatedFigure}
        \end{center}
        \item Spustí se instalační proces.
        \item \vbox{Je možné, že instalátor se Vás zeptá, zda mu důvěřujete; stiskněte ano.
        \begin{center}
            \begin{annotatedFigure}
                {\includegraphics[width=0.75\linewidth]{installer_trust}}
                \draw[red,thick,rounded corners] (0.595,0.198) rectangle (0.79,0.29);
            \end{annotatedFigure}
        \end{center}}
        \item Počkejte, až se instalace dokončí.
        \item Po dokončení instalace se spustí program \appname{}.
    \end{enumerate}
    \begin{infoBox}
        Vzhled ilustrací nemusí v~závislosti na konfiguraci Vašeho systému zcela odpovídat výstupu na Vaši obrazovce.
    \end{infoBox}
    \joinBox{}
    \begin{infoBox}
        Po dokončení instalace se vytvoří zástupce na ploše a~záznamy v~nabídce Start.
    \end{infoBox}
    \joinBox{}
    \begin{infoBox}
    Tento software vyžaduje mít nainstalovaný \emph{.NET Framework 4.7.2}. Pokud nemáte \emph{.NET Framework 4.7.2} nainstalován, instalátor ho stáhne a~nainstaluje (je nutné internetové připojení.)
    \begin{warningBox}Váš poskytovatel internetového připojení Vám může účtovat poplatky za připojení k~internetu související se stažením \emph{.NET Framework 4.7.2}.\end{warningBox}
    \end{infoBox}
    \section{Spuštění}
    \subsection{Pomocí nabídky Start}
        \label{sec:launch_startmenu}
        \begin{tabularx}{\textwidth}{l X}
        \raisebox{-5em}{\includegraphics[scale=1.5]{startmenu_icon.png}} & V~nabíde Start přejděte do složky \emph{Lidé u~výtahu} a~vyberte položku \emph{\appname{}}.
        \end{tabularx}
    \subsection{Pomocí ikonky na ploše}
        \begin{tabularx}{\textwidth}{l X}
            \raisebox{-4em}{\includegraphics[scale=1.25]{desktop_icon}} & Poklepejte na ikonku \emph{\appname{}} na ploše.
        \end{tabularx}
        \begin{infoBox}
            Ikonka nemusí být na ploše, pokud jste ji smazal(a) nebo máte skryté ikonky na ploše. V~tom případě můžete stále využít možnost \uv{Pomocí nabídky start} \nref{sec:launch_startmenu}.
        \end{infoBox}
    \section{Odinstalace}
    \begin{enumerate}
        \item Pro odinstalaci možnost \uv{odinstalovat} v~sekci \uv{Přidat a~odebrat programy} v~operačním systému Windows. Umístění a~název sekce a~název možnosti se může lišit na verzi vašeho operačního systému Windows; konzuljte dokumentaci Vaši verze operačního systému Windows.
        \item V~dialogu odinstalátoru zvolte (pokud není zvolená) možnost \uv{Odeberte aplikaci z~tohot počítače.} a~potvrďte tlačítkem \uv{OK}.
        \begin{center}
            \begin{annotatedFigure}
                {\includegraphics[width=0.75\linewidth]{uninstaller_dialog}}
                \draw[red,thick,rounded corners] (0.118,0.3178) rectangle (0.5855,0.42);
                \draw[red,thick,rounded corners] (0.355,0.03) rectangle (0.5325,0.15);
            \end{annotatedFigure}
        \end{center}
    \end{enumerate}
    \section{Užívání}
    ...
    \vbox{V~této sekci jsou popsány funkce všech \uv{kláves funckí a~operací} \fref{fig:UI_keyboard}{.F}.
    ...
    \begin{function_reference}{btn_divide}
        ...
        \begin{infoBox}
            Pro rychlý výpočet převrácené hodnoty lze využít funkci \uv{převrácená hodnota} \nref{sec:inverse}.
        \end{infoBox}
        \joinBox
        \begin{prohibitBox}
    ...
    \subsubsection{Převrácená hodnota}
    \label{sec:inverse}
    \begin{function_reference}{btn_inverse}
        Vypočíta převrácenou hodnotu operandu.
        \begin{infoBox}
            Tato operance je ekvivalentní k~operaci $1 / X$ \nref{sec:division}.
        \end{infoBox}
        \joinBox
        \begin{prohibitBox}
            Převrácená hodnota nuly není definované a~způsobí chybové hlášení.
        \end{prohibitBox}
    \end{function_reference}
    ...
    Stisknutím klávesy \uv{F1} v~otevřeném programu \appname{} se zobrazí elektronická verze této uživatelské příručky; verze tištěné a~elektronické příručky se mohou lišit.
    ...
    Stisknutím klávesy \uv{F2} v~otevřeném programu \appname{} se zobrazí informace o~programu, jeho autorech a~verzi.
    ...
    \begin{tabularx}{\textwidth}{X X}
        \raisebox{-4.25em}{\includegraphics[scale=0.25]{logo_fit.pdf}} & Tento software byl vyvinut jako projekt předmětu IVS studenty Fakulty informačních technoligií Vysokéhu učení technického v~Brně.
    \end{tabularx}
    
    \section{Historie dokumentu}
    ...
    \vbox{Tato sekce se váže k~softwaru, nikoliv k~dokumentaci.
    
    \begin{otherlanguage}{english}
    \appname{} (Simple calculator with GUI and mathematical library)\\
    ...
    along with \appname{}.  If not, see \url{https://www.gnu.org/licenses/}.
    \end{otherlanguage}}
